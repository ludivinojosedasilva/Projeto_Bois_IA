...
def verificar_se_e_boi(img_input, modelo):
    """Valida se o peso estimado está dentro de faixa plausível"""
...
if escolha == "Nova Pesagem":
    ...
    with col_input:
        ...
        foto = st.file_uploader("Selecione a foto (Vista Traseira)", type=['jpg', 'jpeg', 'png'])
    ...
    if foto is not None:
        ...
        if st.button("🚀 Calcular Peso"):
            with st.spinner('Aguarde, a IA está processando...'):
                model = load_model_ia()
                ...
                if model:
                    ...
                    if not valido:
                        st.error(f"❌ Imagem Rejeitada: O peso estimado de {peso_final:.2f}kg é inconsistente.")
                    else:
                        # Ajuste: somar +70 kg
                        peso_corrigido = peso_final + 70
                        ...
                        c.execute("INSERT INTO pesagens (brinco_id, data, peso_estimado, caminho_foto) VALUES (?, ?, ?, ?)", 
                                  (brinco, agora, peso_corrigido, nome_arq_foto))
                        conn.commit()
                        conn.close()
                        
                        st.success("✅ Pesagem registrada com sucesso!")
                        st.metric(label="Peso Calculado", value=f"{peso_corrigido:.2f} kg")
    ...
    if os.path.exists(DB_PATH):
        ...
        if not df.empty:
            ...
            id_audit = st.selectbox("Escolha o ID da pesagem:", df['id'])
